...
int kmeans(double **T, int k, int N) {
    ...
    while (1) {
        for (i = 0; i < N; ++i) {
            set_cluster(i, k, points_to_cluster, centroids, k);
        }
        changed = update_centroids(k, N, points_to_cluster, centroids, k);
        iters++;
...
double distance(double *p, double *centroids, int cluster, int k) {
    double d = 0;
    int i;
    for (i = 0; i < k; ++i) {
        double multi;
...
double *cluster_mean(int cluster, int *c2p, double *p2c, int k, int num_of_points) {
    int size;
    double val;
    double p_val;
    int i, j;
...
int equal(double *arr1, double *arr2, int k) {
    int i;
    for (i = 0; i < k; ++i) {
        if (arr1[i] != arr2[i]) {
            return 0;
